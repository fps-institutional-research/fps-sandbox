"""
This test script is used to output the data from the OneRoster API to NDJSON files on the desktop.
"""
...
log_filename = os.path.join(desktop_path, f"oneroster_pipeline_{timestamp}.log")

# Configure logging to write to both the terminal and the timestamped .log file
logging.basicConfig(
    level=logging.INFO,
...
# Fetch the current token and subscription key from Secret Manager
...
ONEROSTER_ACCESS_TOKEN_DICT = {'access_token': ONEROSTER_ACCESS_TOKEN, 'token_type': 'Bearer'}
...
def discover_endpoints(endpoints: list[str], session: OAuth2Session) -> list[str]:
    """
...
def write_ndjson_file(element):
    """
    Write grouped items to disk as NDJSON.
    element = (endpoint_name, [items_list_page1, items_list_page2, ...])
    """
...
def run_pipeline():
    ...
    discovery_session = OAuth2Session(token=ONEROSTER_ACCESS_TOKEN_DICT)
    ...
    options = PipelineOptions(flags=[
        "--runner=DirectRunner",
        "--direct_num_workers=16" # number of cores * 2
        ])
        ...
        _ = (
            ...
            | "Fetch Pages" >> RequestResponseIO(
                caller=OneRosterPageCaller(
                    token_dict=ONEROSTER_ACCESS_TOKEN_DICT,
            ...
            | "Write NDJSON" >> beam.Map(write_ndjson_file)
        )
